...
/*
 * Kihyun Yoon <syous95@naver.com>
 *
 * Based (loosely) on the UBOOT code
 */
...
struct dm_mmc_ops {
	 ...
	 */
	int (*deferred_probe)(struct device *dev);
	/**
	 ...
	 * @return 0 if OK, -ve on error
	 */
	int (*execute_tuning)(struct device *dev, uint32_t opcode);
#endif
	 ...
	 */
	int (*wait_dat0)(struct device *dev, int state, int timeout_us);

#ifdef MMC_HS400_ES_SUPPORT
	/* set_enhanced_strobe() - set HS400 enhanced strobe */
	int (*set_enhanced_strobe)(struct device *dev);
#endif
	 ...
	 * @return 0 if not present, 1 if present, -ve on error
	 */
	int (*host_power_cycle)(struct device *dev);
	 ...
	 */
	//int (*get_b_max)(struct device *dev, void *dst, lbaint_t blkcnt);
	 ...
	 */
	int (*hs400_prepare_ddr)(struct device *dev);
};
...
static inline bool mmc_is_mode_ddr(enum bus_mode mode)
{
	if (mode == MMC_DDR_52)
		...
		return true;
#endif
#ifdef CONFIG_MMC_HS400_SUPPORT
...
static inline bool supports_uhs(uint32_t caps)
{
#ifdef CONFIG_MMC_UHS_SUPPORT
...
struct mmc {
	...
	struct device *dev;	/* Device for this MMC controller */
#if CONFIG_IS_ENABLED(DM_REGULATOR)
	struct device *vmmc_supply;	/* Main voltage regulator (Vcc)*/
	struct device *vqmmc_supply;	/* IO voltage regulator (Vccq)*/
#endif
#endif
#endif
	uint8_t *ext_csd;
 ...
 * @return 0 if OK, -ve on error
 */
int mmc_of_parse(struct device *dev, struct mmc_config *cfg);
 ...
 * @return 0 if OK, -ve on error
 */
int mmc_pwrseq_get_power(struct device *dev, struct mmc_config *cfg);
#endif
 ...
 * @return 0 if OK, -ve on error
 */
int mmc_set_clock(struct mmc *mmc, uint32_t clock, bool disable);
...
int mmc_set_part_conf(struct mmc *mmc, uint8_t ack, uint8_t part_num, uint8_t access);
/* Function to modify the BOOT_BUS_WIDTH field of EXT_CSD */
int mmc_set_boot_bus_width(struct mmc *mmc, uint8_t width, uint8_t reset, uint8_t mode);
/* Function to modify the RST_n_FUNCTION field of EXT_CSD */
int mmc_set_rst_n_function(struct mmc *mmc, uint8_t enable);
