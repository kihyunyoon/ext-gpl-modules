			...
			bool send_status)
{
	...
	do {
		...
		if (!ret && (status & MMC_STATUS_RDY_FOR_DATA) &&
			...
			return 0;
		k_busy_wait(100);
	} while (1/*get_timer(start) < timeout_ms*/);
			      ...
			      bool hsdowngrade)
{
...
#else
	uint8_t test_csd[MMC_MAX_BLOCK_LEN] __aligned(64);
#endif
	...
	case MMC_HS_400:
		...
		break;
#endif
#if defined(MMC_HS400_ES_SUPPORT)
	case MMC_HS_400_ES:
		speed_bits = EXT_CSD_TIMING_HS400;
		break;
#endif
...
#if defined(MMC_HW_PARTITIONING)
...
static int sd_get_capabilities(struct mmc *mmc)
{
	...
	struct mmc_cmd cmd;
#if 0
	ALLOC_CACHE_ALIGN_BUFFER(sys_be32, scr, 2);
	ALLOC_CACHE_ALIGN_BUFFER(sys_be32, switch_status, 16);
#else
	uint32_t scr[2] __aligned(64);
	uint32_t switch_status[16] __aligned(64);
#endif
	struct mmc_data data;
	int timeout;
	...
	mmc->scr[0] = sys_be32_to_cpu(scr[0]);
	mmc->scr[1] = sys_be32_to_cpu(scr[1]);
	...
	while (timeout--) {
		...
		if (!(sys_be32_to_cpu(switch_status[7]) & SD_HIGHSPEED_BUSY))
			break;
	}

	/* If high-speed isn't supported, we return */
	if (sys_be32_to_cpu(switch_status[3]) & SD_HIGHSPEED_SUPPORTED)
	...
	sd3_bus_mode = sys_be32_to_cpu(switch_status[3]) >> 16 & 0x1f;
...
static int sd_set_card_speed(struct mmc *mmc, enum bus_mode mode)
{
	...
	if (((sys_be32_to_cpu(switch_status[4]) >> 24) & 0xF) != speed)
		return -ENOTSUP;
...
#else
static inline int mmc_set_signal_voltage(struct mmc *mmc, uint32_t signal_voltage)
{
	return 0;
}
#endif
...
static int mmc_startup_v4(struct mmc *mmc)
{
	...
	ALLOC_CACHE_ALIGN_BUFFER(uint8_t, ext_csd, MMC_MAX_BLOCK_LEN);
#else
	uint8_t ext_csd[MMC_MAX_BLOCK_LEN] __aligned(64);
#endif
...
#if defined(CONFIG_MMC_UHS_SUPPORT) || \
